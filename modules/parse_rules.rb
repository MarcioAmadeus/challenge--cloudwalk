module ParseRules
  ...
  def self.killer_rule # to be used after parsing the kill line with the kill_rule 
    /(?<=:\s)(.*?)(?=\skilled)/
  end
  
  def self.killed_rule # to be used after parsing the kill line with the kill_rule 
    /(?<=killed\s)(.*?)(?=\sby)/
  end
  
  def self.death_reason_rule # to be used after parsing the kill line with the kill_rule 
    /(?<=by\s)(.*?)(?=$)/
  end
  ...
  def self.player_name_rule # to be used after parsing the player connect line with the player_connect_rule
    /(?<=\\)(.*?)(?=\\)/
  end
